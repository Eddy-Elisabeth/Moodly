...
import React, { useState } from 'react';
import { View, TextInput, Button, Text, Alert, TouchableOpacity, StyleSheet, Image, ScrollView } from 'react-native';
import axios from 'axios';
import AsyncStorage from '@react-native-async-storage/async-storage';

const moodOptions = [
  { label: 'Pet’ la forme', value: 'Heureux', color: '#FFE082', emoji: require('../asset_front/4.png') },
  { label: 'Dans le mood', value: 'Content', color: '#C8E6C9', emoji: require('../asset_front/5.png') },
  { label: 'Dans le mou', value: 'Maussade', color: '#F8BBD0', emoji: require('../asset_front/2.png') },
  { label: 'Aigris', value: 'En colère', color: '#FFAB91', emoji: require('../asset_front/3.png') },
  { label: 'Triste', value: 'Triste', color: '#B3E5FC', emoji: require('../asset_front/1.png') },
  { label: 'Blasé', value: 'Blasé', color: '#F3E5AB', emoji: require('../asset_front/6.png') }, // Couleur et image pour Blasé
  { label: 'Fatigué', value: 'Fatigué', color: '#D1C4E9', emoji: require('../asset_front/7.png') }, // Couleur et image pour Fatigué
];
    ...
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: selectedMood ? selectedMood.color : '#FFF' }]}>
      ...
      {moodOptions.map((mood) => (
        <TouchableOpacity
          ...
          onPress={() => setSelectedMood(mood)}
        >
          <Text style={[styles.moodButtonText, selectedMood?.value === mood.value && styles.selectedMoodText]}>
            {mood.label}
          </Text>
        </TouchableOpacity>
      ))}
